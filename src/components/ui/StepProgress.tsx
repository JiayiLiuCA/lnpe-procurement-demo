...
//  - StepStepper：项目详情头部。可点击切换查看的步骤；下方款项轨 4 个节点各自对齐在触发它的步骤下面。
...
export function StepStepper({
  ...
  return (
    <div className="flex flex-col gap-2.5" style={{ width: COL_W * STEP_NOS.length }}>
      {/* 货物轨 */}
              ...
              : s.mark === "current"
                ...
                : s.mark === "done"
                  ...
                  : s.mark === "partial"
                    ...
                    : "text-faint";
          return (
            <button key={s.no} type="button" onClick={() => onSelect(s.no)} className="relative flex cursor-pointer flex-col items-center gap-1.5" style={{ width: COL_W }}>
              {i < states.length - 1 && (
              ...
              <span className={`h-[14px] text-[11px] leading-[14px] whitespace-nowrap ${s.mark === "issue" ? "text-danger-deep" : "text-sub"}`}>
      ...
      {/* 付款开票轨：节点对齐在触发它的步骤下方 */}
      <div className="flex items-start">
        <div className="text-sub flex items-center justify-end gap-1.5 pr-4 text-[11.5px]" style={{ width: COL_W * 3, height: 40 }}>
          <span className="bg-line-soft h-px flex-1" />
          <span className="whitespace-nowrap font-medium">付款开票</span>
          <span className="text-faint whitespace-nowrap">按合同付款条款</span>
        </div>
        {lane.map((n) => {
          ...
          return (
            <button
              ...
              className={`flex cursor-pointer items-center justify-center gap-2 rounded-lg py-1.5 ${active ? "bg-line-soft" : "hover:bg-page"}`}
              style={{ width: COL_W, height: 40 }}
            >
              <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ${tone.dot}`}>¥</span>
              <span className="flex flex-col items-start leading-tight">
                <span className="text-[11.5px] font-medium whitespace-nowrap">
                  {n.label} {Math.round(n.ratio * 100)}%
                </span>
                <span className={`text-[10.5px] whitespace-nowrap tabular-nums ${tone.text}`}>
                  {n.total === 0 ? "—" : `${n.paid}/${n.total} 已付${n.overdue > 0 ? " · 逾期" : n.due > 0 ? " · 到期" : ""}`}
                </span>
              </span>
            </button>
          );
