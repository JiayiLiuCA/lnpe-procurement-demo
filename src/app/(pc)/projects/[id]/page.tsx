...
// 头卡只放标题 / 派生标签 / 步骤条 + 款项轨，不再有逐步说明条：每步要做的事由正文的主按钮与空状态说清，
// 项目级金额挂在款项轨表头，各步的辅助入口（批次切换 / 查看库存 / 收货记录 / 小程序）放进正文标题行。
// 步骤 2（清单卡 + 批次版本记录 + 只读明细）在 components/pc/ChecklistStep；步骤 3 订货安排 = 清单表 + 标库存 / 生产 / 需采购 + 出合同，
// 批次固定可选（一般只有一批），未批准的批次在步骤 3 只显示「待批准」空状态；步骤 4 子合同只放合同卡与覆盖汇总，不再画清单表。
import { useState } from "react";
import Link from "next/link";
import { useParams, useSearchParams } from "next/navigation";
import { AlertTriangle, Archive, Check, List, Smartphone, Truck } from "lucide-react";
...
import { ChecklistStep, batchLabel } from "@/components/pc/ChecklistStep";
...
import { contractStage, isSignedContract, projectDeliveryNotes, projectProgress, type StepNo } from "@/lib/steps";
...
export default function ProjectDetailPage() {
  ...
  // 需采购项的合同覆盖（只算已批准批次）：need = 采购项合计，contracted = 已入合同，signedRows = 其中已签合同覆盖的
  const signedIds = new Set(signedList.map((c) => c.id));
  const coverage = checklists
    .filter((c) => c.status === "已批准")
    .reduce(
      (acc, c) => {
        const st = checklistStats(c);
        const signedRows = c.sheets.flatMap((sh) => sh.rows).filter((r) => r.contractId && signedIds.has(r.contractId)).length;
        return { need: acc.need + st.need, contracted: acc.contracted + st.contracted, signedRows: acc.signedRows + signedRows };
      },
      { need: 0, contracted: 0, signedRows: 0 },
    );
  const coverPct = (n: number) => `${(n / Math.max(coverage.need, 1)) * 100}%`;
  ...
  const deadlineDays = project.deliveryDeadline && !project.closedAt ? daysUntil(project.deliveryDeadline) : null;
  ...
  // 批次选择（订货安排 / 子合同步）：一般只有一批，仍固定显示，便于按批次筛看清单内容
  const batchTabs =
    checklists.length > 0 ? (
      <div className="flex items-center gap-2">
        ...
        {checklists.map((c, i) => (
          <button
            ...
            type="button"
            onClick={() => setBatch(i)}
            className={`cursor-pointer rounded-full px-3 py-1 text-[12.5px] font-medium ${i === batch ? "chip-selected" : "border-line text-ink-2 border bg-white"}`}
          >
            {batchLabel(c)} · {c.status === "已批准" ? `${checklistStats(c).total} 项` : "待批准"}
          </button>
        ))}
      </div>
    ) : null;
  ...
  /** 步骤 3：所选批次尚未批准时不开放操作 */
  const pendingApproval = (extra?: React.ReactNode) => (
    <div className={`${sectionCard} flex flex-col gap-3 px-5 py-4`}>
      <div className="flex items-center gap-2">
        {batchTabs}
        {extra}
      </div>
      <div className="flex flex-col items-center py-6">
        <EmptyState
          ...
          text={cl ? `${batchLabel(cl)}清单 ${cl.versions[cl.versions.length - 1]?.id} 待批准 · 批准后开始订货安排` : ""}
        />
        <Btn variant="primary" onClick={() => setViewStep(2)}>
          去批准
        </Btn>
      </div>
    </div>
    ...
    <div className="flex min-h-dvh flex-col">
      ...
      <div className="flex flex-1 flex-col gap-3.5 p-6">
        ...
        <div className="border-line rounded-card overflow-hidden border bg-white">
          <div className="flex flex-col gap-4 px-5.5 pt-4 pb-4">
            <div className="min-w-0">
              ...
              <div className="text-sub mt-1.5 text-[12.5px]">
                ...
                {project.deliveryDeadline ? (
                  <span className={deadlineDays != null && deadlineDays < 0 ? "text-danger-deep font-medium" : ""}>
                    {project.deliveryDeadline}
                    {deadlineDays != null && (deadlineDays < 0 ? ` · 逾期 ${-deadlineDays} 天` : ` · 剩 ${deadlineDays} 天`)}
                  </span>
                ) : (
        ...
        {/* ---------- 2 采购清单：清单卡（批准 / 上传新版本 / 追加批次 / 查看清单）+ 批次版本记录 + 只读明细 ---------- */}
        {step === 2 && <ChecklistStep project={project} checklists={checklists} batch={batch} onSelectBatch={setBatch} />}
            ...
            {contracts.length === 0 ? (
              <div className={`${sectionCard} flex flex-col items-center py-10`}>
                <EmptyState
                  ...
                  text={
                    coverage.need > 0
                      ? `${coverage.need} 项需采购尚未出合同 · 在「订货安排」步勾选需采购行，AI 生成合同初稿或上传已签合同`
                      : "订货安排完成后，需采购项在该步生成子合同"
                  }
                />
                <Btn variant="primary" onClick={() => setViewStep(3)}>
                  去订货安排出合同
                </Btn>
              </div>
            ) : (
              <>
                {/* 合同进程：采购项按 已签合同覆盖 / 草稿覆盖 / 尚未出合同 三段，右侧大数字 = 已入合同 x/y 项 */}
                <div className={`${sectionCard} flex items-center gap-5 px-5 py-3.5`}>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 text-[13px]">
                      <span className="font-bold">合同进程</span>
                      <span className="text-ink-2">
                        子合同 <span className="text-ink font-bold">{contracts.length}</span> 份 · 已签{" "}
                        <span className="text-success-deep font-bold">{signedList.length}</span> 份 · 初稿 / 校对 / 定稿{" "}
                        <span className={`font-bold ${contracts.length - signedList.length > 0 ? "text-info-deep" : "text-faint"}`}>{contracts.length - signedList.length}</span> 份
                      </span>
                      <span className="text-sub text-xs">校对、定稿、签订在各子合同页完成</span>
                      <div className="flex-1" />
                      {coverage.need - coverage.contracted > 0 && (
                        <button type="button" onClick={() => setViewStep(3)} className="text-warning-deep cursor-pointer text-[12.5px] font-medium">
                          还有 {coverage.need - coverage.contracted} 项需采购 · 去订货安排出合同
                        </button>
                      )}
                    ...
                    <div className="bg-line-soft mt-2 flex h-2 gap-px overflow-hidden rounded">
                      <div className="bg-success" style={{ width: coverPct(coverage.signedRows) }} />
                      <div className="bg-info" style={{ width: coverPct(coverage.contracted - coverage.signedRows) }} />
                      <div className="bg-warning" style={{ width: coverPct(coverage.need - coverage.contracted) }} />
                    </div>
                    <div className="text-sub mt-1.5 flex gap-4 text-[11.5px]">
                      <span>
                        <span className="bg-success mr-1 inline-block h-2 w-2 rounded-sm align-middle" />
                        已签合同覆盖 {coverage.signedRows} 项
                      </span>
                      <span>
                        <span className="bg-info mr-1 inline-block h-2 w-2 rounded-sm align-middle" />
                        草稿覆盖 {coverage.contracted - coverage.signedRows} 项
                      </span>
                      <span>
                        <span className="bg-warning mr-1 inline-block h-2 w-2 rounded-sm align-middle" />
                        需采购 {coverage.need - coverage.contracted} 项
                      </span>
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className="text-sub text-xs">已入合同</div>
                    <div className="text-[19px] font-bold tabular-nums">
                      <span className={coverage.contracted >= coverage.need ? "text-success-deep" : "text-info-deep"}>{coverage.contracted}</span>
                      <span className="text-sub text-[13px] font-medium">/{coverage.need} 项</span>
                    </div>
                  </div>
                </div>
            ...
            {delivering.length > 0 ? (
              ...
              <div className={`${sectionCard} flex py-8`}>
                <EmptyState
                  ...
                  text={signedList.length === 0 ? "尚无已签子合同 · 先在「采购合同」步签订" : "全部子合同货已到场 · 转到现场收货"}
                />
              </div>
            )}
            {beyond.length > 0 && (
        ...
        {step === 6 && (
          <>
            <div className="flex items-center gap-2">
              <div className="text-[15px] font-bold">
                送货单 <span className="text-sub text-[12.5px] font-normal">{pNotes.length} 张 · 现场按送货单逐行打勾、拍照留存，异常在这里处理</span>
              </div>
              <div className="flex-1" />
              <Link href="/receipts">
                <Btn variant="secondary" size="sm">
              ...
              <div className="flex items-center gap-2.5">
                ...
                {signedList.length > 0 && (
                  <div className="text-ink-2 text-[12.5px] tabular-nums">
                    已签 {signedList.length} 份 · 已签总额 <Money value={signedTotal} className="text-ink font-medium" /> · 已付{" "}
                    <span className="text-ink font-medium">{paidPct}</span> · 到期未付{" "}
                    <span className={`font-medium ${projectPlan.length > 0 ? "text-warning-deep" : "text-success-deep"}`}>{projectPlan.length} 笔</span>
                  </div>
                )}
              </div>
              {signedList.length > 0 && (
                <div className="border-line-soft overflow-hidden rounded-[10px] border">
                  <div className="text-sub border-line-soft flex border-b bg-[#FBFAF9] px-4 py-2 text-xs font-medium">
                    <div className="w-[200px]">采购合同</div>
                    ...
                    return (
                      <div key={c.id} className={`flex items-center px-4 py-2.5 text-[13px] ${i < signedList.length - 1 ? "border-page border-b" : ""}`}>
                        ...
                        <div className="text-ink-2 flex-1 text-[12.5px]">
                          {c.status === "closed" ? "已关闭" : c.status === "warranty" ? "质保期" : c.status === "arrived" ? `货到 ${fmtDate(c.goodsArrivedAt)}` : "未到货"}
                        </div>
                      </div>
                    );
                  })}
